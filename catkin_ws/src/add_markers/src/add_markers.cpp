...
int main( int argc, char** argv )
{
  ...
  while (ros::ok())
  {
    ...
    // Get the odometry value here
    float distance;
    distance = 10;
    boost::shared_ptr<nav_msgs::Odometry const> odom;
    std::stringstream buffer;
    while(distance > 0.01){
      ROS_INFO("Getting odometry values");
      odom = ros::topic::waitForMessage<nav_msgs::Odometry>("/odom");
      distance = abs(odom->pose.pose.position.x - 3.75);
      distance += abs(odom->pose.pose.position.y - 8.0);
      buffer << distance;
      ROS_INFO("%s\n", buffer.str().c_str());
      buffer.str("");
    }
    ...
    // make marker appear at target position
    marker.pose.position.x = -3.75;
    marker.pose.position.y = 8.0;
